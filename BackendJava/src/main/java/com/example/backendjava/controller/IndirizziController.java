...
public class IndirizziController {
    ...
    public ResponseEntity<?> create(@RequestHeader(value = "Authorization", required = false) String authorization,
        ...
        try {
            ...
            SqlRowSet checkDuplicate = jdbc.queryForRowSet(
                "SELECT * FROM indirizzi WHERE user_id = ? AND LOWER(via) = LOWER(?) AND LOWER(citta) = LOWER(?) AND cap = ?",
            ...
            if (Boolean.TRUE.equals(isDefault)) {
                try {
                    jdbc.update("UPDATE indirizzi SET is_default = ? WHERE user_id = ?", false, userId);
                } catch (Exception e) {
            ...
            SqlRowSet rs = jdbc.queryForRowSet(
                "INSERT INTO indirizzi (user_id, via, citta, cap, provincia, nazione, is_default, nome_destinatario, telefono_destinatario) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) RETURNING *",
    ...
    private Map<String, Object> mapIndirizzoToFrontend(SqlRowSet rs) {
        ...
        row.put("id", rs.getObject("id_indirizzo"));
        row.put("indirizzo", rs.getObject("via"));
        row.put("citta", rs.getObject("citta"));
        row.put("cap", rs.getObject("cap"));
        row.put("provincia", rs.getObject("provincia"));
        row.put("paese", rs.getObject("nazione"));
        row.put("predefinito", rs.getObject("is_default"));
        row.put("destinatario", rs.getObject("nome_destinatario"));
        row.put("telefono", rs.getObject("telefono_destinatario"));
    ...
    public ResponseEntity<?> setDefault(@RequestHeader(value = "Authorization", required = false) String authorization,
        ...
        long userId = maybeUser.get();
        jdbc.update("UPDATE indirizzi SET is_default = ? WHERE user_id = ?", false, userId);
        jdbc.update("UPDATE indirizzi SET is_default = ? WHERE id_indirizzo = ? AND user_id = ?", true, indirizzoId, userId);
    ...
    public ResponseEntity<?> update(@RequestHeader(value = "Authorization", required = false) String authorization,
        ...
        SqlRowSet owner = jdbc.queryForRowSet("SELECT 1 FROM indirizzi WHERE id_indirizzo = ? AND user_id = ?", indirizzoId, userId);
        ...
        SqlRowSet checkDuplicate = jdbc.queryForRowSet(
            "SELECT * FROM indirizzi WHERE user_id = ? AND id_indirizzo != ? AND LOWER(via) = LOWER(?) AND LOWER(citta) = LOWER(?) AND cap = ?",
        ...
        SqlRowSet rs = jdbc.queryForRowSet(
                "UPDATE indirizzi SET via = ?, citta = ?, cap = ?, provincia = ?, nazione = ?, is_default = ?, nome_destinatario = ?, telefono_destinatario = ? WHERE id_indirizzo = ? AND user_id = ? RETURNING *",
    ...
    public ResponseEntity<?> delete(@RequestHeader(value = "Authorization", required = false) String authorization,
        ...
        if (maybeUser.isEmpty()) return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(Map.of("message", "Token mancante o non valido"));
        long userId = maybeUser.get();
        SqlRowSet rs = jdbc.queryForRowSet("DELETE FROM indirizzi WHERE id_indirizzo = ? AND user_id = ? RETURNING *", indirizzoId, userId);
